...
### express server 和 webpack compiler 生成

- vue-cli 在本地开发模式下，为什么采用 express 启动静态资源服务器？
  - 解决线上部署后的资源路径问题 `publicPath: './'`
  - 解决 history 模式下的 URL fallback 问题 （就是在 history 路由模式下，出现 404 问题，express 可以解决这个问题 ）
...
- 前端项目为什么需要使用脚手架？
  ...
  - 如果我们没有脚手架时会怎么使用 Vue ？
    - 创建项目：拷贝或下载项目模版 VS 交互式项目创建
    - 项目运行和打包：编写 Webpack 配置、启动和打包脚本 VS 脚手架自带启动和打包命令
    - 项目框架：手动完成 Vue 全家桶引入和框架搭建 VS 自动生成框架模版代码
- VueCLI 和 CRA 有什么局限？
  - 只能解决本技术域的问题，不同技术域就需要生产不同的脚手架
  - 比如说：一个公司、集团中，技术栈肯定是不一样的，有的是 Vue，有的是 React。如何做到统一？
- 如何解决大前端团队工程链路的统一问题？
...
- `drwxr-xr-x`
- r: 访问 w: 编辑 x: 执行
- u: 当前登陆用户 g: 当前登陆用户所在分组 o: 当前用户所在分组的其他用户
- Unix 使用 32 位二进制数存储文件类型和权限
- 0000 0000 0000 0000
